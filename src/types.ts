...
export interface Vocab {
  ...
  trad: string; // 你好  (the learner's KNOWN language — Cantonese gloss)
  jyut: string; // "nei5 hou2"  (Jyutping for the Cantonese gloss)
  en: string; // "Hello"  (the TARGET being taught — shown as the hero)
  ipa?: string; // "/həˈləʊ/"  (RP pronunciation aid, sits where Jyutping did)
  emoji?: string; // decorative only, unused
...
export interface Sentence {
  /** tiles in answer order: English word + IPA for each tile (the answer to build) */
  tokens: { t: string; j: string }[];
  /** the English sentence (spoken in the "type what you hear" variant) */
  en: string;
  /** the Cantonese sentence shown as the prompt in "build the sentence" */
  cn: string;
